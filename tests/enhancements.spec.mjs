...
async function runEnhancementTests() {
  console.info("Testing message enhancement system...\n");
  ...
  for (const testCase of testCases) {
    ...
    const injectedCorrect =
      hasInjectedResults === testCase.expectInjectedResults;
    ...
    if (testPassed) {
      ...
      console.info(`   Query: "${testCase.query}"`);
      console.info(`   Matched rules: [${matchedRuleIds.join(", ")}]`);
    } else {
      failed++;
      failures.push(testCase);
      console.info(`❌ ${testCase.description}`);
      console.info(`   Query: "${testCase.query}"`);
      console.info(`   Expected rules: [${expectedRuleIds.join(", ")}]`);
      console.info(`   Got rules: [${matchedRuleIds.join(", ")}]`);
      if (!queryEnhancedCorrect) {
        console.info(
          `   Query enhancement: expected ${testCase.expectEnhancedQuery}, got ${queryEnhanced}`,
        );
      }
      if (!injectedCorrect) {
        console.info(
          `   Injected results: expected ${testCase.expectInjectedResults}, got ${hasInjectedResults}`,
        );
      }
    }
  }
...
async function testPriorityOrdering() {
  console.info("\n" + "=".repeat(60));
  console.info("Testing enhancement priority ordering...\n");
  ...
  if (matchedRules.length > 1) {
    const creatorIndex = matchedRules.findIndex(
      (r) => r.id === "creator-author",
    );
    ...
    if (creatorFirst) {
      console.info(
        `✅ Priority ordering correct: creator rule processed first`,
      );
    } else {
...
async function testUrlPrioritization() {
  console.info("\n" + "=".repeat(60));
  console.info("Testing URL prioritization...\n");
  ...
  if (hasPrioritizedUrls) {
    console.info(`✅ URL prioritization correct for creator query`);
    console.info(`   Prioritized URLs: ${result.prioritizedUrls.join(", ")}`);
  } else {
...
(async () => {
  try {
    ...
    console.info("\n" + "=".repeat(60));
    console.info("✅ All enhancement system tests completed successfully!");
  } catch (error) {
