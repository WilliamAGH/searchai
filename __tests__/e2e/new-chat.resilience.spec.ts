...
test.describe("New Chat Resilience", () => {
  ...
  test("should handle navigation failures gracefully", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.history.pushState = function (...args) {
        navCount++;
        if (navCount === 1) {
          console.error("[ERROR] Navigation blocked intentionally for test");
    ...
    await page.waitForLoadState("networkidle", { timeout: 5000 }).catch(() => {
      // Navigation may have been blocked - this is expected in this test
    });
  ...
  test.fixme("should handle browser back/forward navigation", async ({
    ...
    if (canGoBack) {
      ...
      await page
        .waitForURL((url) => url.toString() === firstChatUrl, {
          timeout: 5000,
        })
        .catch(() => {});
