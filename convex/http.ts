...
http.route({
  ...
  handler: httpAction(async (_ctx, request) => {
    ...
    try {
      ...
      console.log('🔍 SEARCH FALLBACK RESPONSE:', JSON.stringify(errorResponse, null, 2));
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      ...
      console.log('🌐 SCRAPE ERROR RESPONSE:', JSON.stringify(errorResponse, null, 2));
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    console.log('Environment Variables Available:');
    console.log('- OPENROUTER_API_KEY:', process.env.OPENROUTER_API_KEY ? 'SET' : 'NOT SET');
    console.log('- CONVEX_OPENAI_API_KEY:', process.env.CONVEX_OPENAI_API_KEY ? 'SET' : 'NOT SET');
    console.log('- CONVEX_OPENAI_BASE_URL:', process.env.CONVEX_OPENAI_BASE_URL ? 'SET' : 'NOT SET');
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      ...
      if (process.env.CONVEX_OPENAI_API_KEY && process.env.CONVEX_OPENAI_BASE_URL) {
        try {
          ...
          console.log('URL:', `${process.env.CONVEX_OPENAI_BASE_URL}/chat/completions`);
          ...
          console.log('🤖 CONVEX OPENAI RESPONSE HEADERS:', Object.fromEntries(response.headers.entries()));

          if (response.ok) {
            const data = await response.json();
            console.log('🤖 CONVEX OPENAI RESPONSE BODY:', JSON.stringify(data, null, 2));
            ...
            console.log('🤖 CONVEX OPENAI SUCCESS RESPONSE:', JSON.stringify(successResponse, null, 2));
            ...
            throw new Error(`Convex OpenAI failed: ${response.status} - ${errorText}`);
          }
        } catch (convexError) {
          console.error('🤖 CONVEX OPENAI EXCEPTION:', convexError);
          console.error('Convex OpenAI Error stack:', convexError instanceof Error ? convexError.stack : 'No stack trace');
        }
      }
      ...
      console.log('🤖 AI FALLBACK RESPONSE:', JSON.stringify(fallbackResponseObj, null, 2));
    ...
    try {
      ...
      const responseContent = aiMessage.content || "I apologize, but I couldn't generate a response. Please try again.";
      ...
      console.log('🤖 OPENROUTER SUCCESS RESPONSE:', JSON.stringify(successResponse, null, 2));
      ...
      if (process.env.CONVEX_OPENAI_API_KEY && process.env.CONVEX_OPENAI_BASE_URL) {
        try {
          ...
          console.log('🤖 CONVEX OPENAI FALLBACK RESPONSE STATUS:', fallbackResponse.status);

          if (fallbackResponse.ok) {
            const fallbackData = await fallbackResponse.json();
            console.log('🤖 CONVEX OPENAI FALLBACK RESPONSE BODY:', JSON.stringify(fallbackData, null, 2));
            
            const responseContent = fallbackData.choices[0].message.content || "I apologize, but I couldn't generate a response.";
            ...
            console.log('🤖 CONVEX OPENAI FALLBACK SUCCESS:', JSON.stringify(fallbackSuccessResponse, null, 2));
          ...
          console.error('Convex OpenAI Fallback Error stack:', convexError instanceof Error ? convexError.stack : 'No stack trace');
        }
      }
      ...
      console.log('🤖 AI FINAL ERROR RESPONSE:', JSON.stringify(finalErrorResponse, null, 2));
