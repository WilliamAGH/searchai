...
function checkFile(filePath) {
  ...
  const isConvex = relativePath.startsWith('convex/');
  const isFrontend = relativePath.startsWith('src/');
  const isTest = relativePath.includes('.test.') || relativePath.includes('.spec.') || relativePath.includes('tests/');
  ...
  const processEnvMatches = [...content.matchAll(PATTERNS.processEnv)];
  const importMetaEnvMatches = [...content.matchAll(PATTERNS.importMetaEnv)];
  const ctxEnvMatches = [...content.matchAll(PATTERNS.ctxEnv)];
  
  if (isConvex) {
    // Check if it's likely an action file (contains 'action' or 'httpAction')
    const isAction = content.includes('action(') || content.includes('httpAction(');
    const isQuery = content.includes('query(');
    const isMutation = content.includes('mutation(');
...
function main() {
  console.log(`${colors.cyan}🔍 Checking environment variable usage patterns...${colors.reset}\n`);
  
  const convexFiles = getAllFiles(join(projectRoot, 'convex'));
  ...
  if (allIssues.length === 0) {
    console.log(`${colors.green}✅ All environment variable usage patterns are correct!${colors.reset}`);
    process.exit(0);
  }
  ...
  for (const [file, issues] of Object.entries(issuesByFile)) {
    ...
    for (const issue of issues) {
      ...
      console.log(`  ${icon} ${color}Line ${issue.line}: ${issue.message}${colors.reset}`);
  ...
  if (errorCount > 0) {
    ...
    console.log(`  • ${colors.bold}Convex actions:${colors.reset} Use process.env for environment variables`);
    console.log(`  • ${colors.bold}Convex queries/mutations:${colors.reset} Cannot access env vars directly`);
    console.log(`  • ${colors.bold}Frontend (src/):${colors.reset} Use import.meta.env for Vite variables`);
    console.log(`  • ${colors.bold}Test files:${colors.reset} Can use either pattern as needed`);
    process.exit(1);
  }
