...
class ConvexTypeChecker {
  ...
  async runTypeScriptCheck() {
    console.log(
      `${colors.blue}🔍 Running TypeScript type checking...${colors.reset}`,
    );
    ...
    try {
      ...
      this.parseTypeScriptOutput(
        mainStdout + mainStderr + convexStdout + convexStderr,
      );
  ...
  async checkConvexPatterns() {
    console.log(
      `${colors.blue}🔍 Checking Convex type patterns...${colors.reset}`,
    );
    ...
    for (const file of files) {
      ...
      if (/interface\s+\w+\s*{\s*_id\s*:/.test(content)) {
        this.errors.push({
          ...
          message:
            "Manual Doc type definition found. Use Doc<TableName> from _generated/dataModel",
        });
      }
      ...
      for (const pattern of functionPatterns) {
        ...
        for (const match of matches) {
          ...
          const functionBlock = this.extractFunctionBlock(
            content,
            functionStart,
          );
      ...
      if (!content.includes('from "./_generated/')) {
        if (content.includes("query") || content.includes("mutation")) {
          this.warnings.push({
            ...
            message:
              "Convex function file should import from _generated directory",
          });
        }
      }
  ...
  printResults() {
    ...
    if (this.errors.length > 0) {
      ...
      for (const error of this.errors) {
        ...
        console.log(
          `    ${colors.red}${error.type}:${colors.reset} ${error.message}`,
        );
      }
    }
