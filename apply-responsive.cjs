...
console.info("🚀 Applying responsive design changes...\n");
...
try {
  ...
  if (!indexCss.includes("@import './styles/responsive.css'")) {
    ...
    console.info("✅ Added responsive CSS import");
  }
} catch {
  console.info("⚠️  Could not add responsive CSS import");
}
...
updateFile("./src/App.tsx", [
  ...
  },
  {
    ...
    replace:
      'className="px-3 sm:px-4 py-1.5 text-xs sm:text-sm font-medium bg-emerald-500',
  },
]);
...
updateFile("./src/components/ShareModal.tsx", [
  {
    ...
    replace:
      'className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-sm sm:max-w-md w-full mx-4 p-5 sm:p-6',
  },
  {
...
try {
  ...
  if (fs.existsSync(authModalPath)) {
    updateFile(authModalPath, [
      {
        ...
        replace:
          'className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-sm sm:max-w-md w-full mx-4 p-6 sm:p-8',
      },
      {
    ...
    console.info(
      "ℹ️  AuthModal.tsx not found; skipping responsive updates for it",
    );
  }
} catch (e) {
  console.info(
    "⚠️  Could not update AuthModal.tsx (optional):",
    e?.message || e,
  );
}
...
console.info("  • Mobile phones (320px+)");
console.info("  • Tablets (768px+)");
console.info("  • Desktops (1024px+)");
console.info("  • 4K displays (2560px+)");
